...
public class SphereMovement : MonoBehaviour
{
	...
	void Update()
	{
		m_MoveVector = Vector2.ClampMagnitude(m_MoveVector, 1f);
		...
		if (playerInputSpace)
		{
			...
			right.Normalize();
			desiredVelocity =
				(forward * m_MoveVector.y + right * m_MoveVector.x) * maxSpeed;
		}
		else
		{
			desiredVelocity =
				new Vector3(m_MoveVector.x, 0f, m_MoveVector.y) * maxSpeed;
		}
